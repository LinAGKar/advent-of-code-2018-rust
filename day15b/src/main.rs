...
fn main() {
    ...
    let mut start_mobs = Vec::new();
    let mut start_mobs_iter = Vec::new();

    let start_map: Vec<Vec<_>> = input.lines().enumerate().map(|(y, line)| {
        line.chars().enumerate().map(|(x, chr)| match chr {
            '#' => Tile::Wall,
            '.' => Tile::Air,
            'E' => {
                let pos = Vec2{y: y, x: x};
                start_mobs.push((pos, Mob::Elf));
                start_mobs_iter.push(pos);
                Tile::Mob
            }
            'G' => {
                let pos = Vec2{y: y, x: x};
                start_mobs.push((pos, Mob::Goblin));
                start_mobs_iter.push(pos);
                Tile::Mob
            }
            _ => panic!(""),
    ...
    'power_level: for power in ATK + 1.. {
        let mut mobs_iter = start_mobs_iter.clone();
        let mut mobs: HashMap<_, _> = start_mobs.iter().map(|&(pos, mob_type)| {
            (pos, (HP, mob_type, -1))
        }).collect();
        ...
        for round in 0.. {
            for (n, &pos) in mobs_iter.iter().enumerate() {
                ...
                if mobs.values().all(|&(_, mob_type, _)| mob_type == Mob::Elf) {
